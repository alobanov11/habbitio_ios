...
struct HabitEditView: View {
  ...
  var body: some View {
    ...
    .onChange(of: category) { oldValue, newValue in
      if newValue.count > 16 {
        category = String(newValue.prefix(16))
      }
    }
    .onDisappear {
...
extension HabitEditView {
  fileprivate var titleFieldView: some View {
  ...
  fileprivate var categoryFieldView: some View {
    VStack(alignment: .leading) {
      Text("Category")
        .font(.system(.callout, design: .monospaced))
        .fontWeight(.bold)
  ...
  fileprivate var daysFieldView: some View {
    VStack(alignment: .leading) {
      Text("Days")
        .font(.system(.callout, design: .monospaced))
        .fontWeight(.bold)
  ...
  fileprivate var reminderView: some View {
    VStack(spacing: 24) {
      HStack {
...
extension HabitEditView {
  fileprivate func done() {
    let habit =
      ...
      ?? {
        let newHabit = Habit(
          title: self.title, category: self.category.isEmpty ? nil : self.category)
        modelContext.insert(newHabit)
        return newHabit
      }()

    habit.title = self.title
    habit.category = self.category.isEmpty ? nil : self.category
